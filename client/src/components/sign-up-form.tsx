...
export function SignUpForm() {
  ...
  const renderAuthStep = () => (
    <CardContent className="space-y-4">
      ...
      {formData.password && (
        <div className="space-y-2">
          <Progress value={getPasswordStrength(formData.password).score * 25} className="h-2" />
      ...
      <Button
        type="submit"
        disabled={!validateEmail(formData.email) || !validatePassword(formData.password) || loading}
        className="w-full"
      >
  ...
  const renderPhoneStep = () => (
    <CardContent className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        <Button type="button" variant="link" onClick={handleResendOtp} disabled={!canResend} className="p-0">
          Resend Code
        </Button>
        {!canResend && <span className="text-sm text-muted-foreground">Resend in {otpTimer}s</span>}
      </div>
      <Button
        type="submit"
