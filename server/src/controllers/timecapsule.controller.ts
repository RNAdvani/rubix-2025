...
export const createCapsule = TryCatch(async (req, res, next) => {
   ...
   if (!title || !description)
      return next(
         new ErrorHandler(400, "Please provide title and description")
      );
...
export const getCreatedCapsules = TryCatch(async (req, res, next) => {
   const capsules = await TimeCapsule.aggregate([
      {
         $match: {
            creator: req.user._id,
         },
            ...
            media: {
               $map: {
                  ...
                  in: {
                     url: "$$m.url",
                     metadata: "$$m.metadata",
                     AIGeneratedSummary: "$$m.AIGeneratedSummary",
                  },
               },
            },
         },
...
export const getCapsule = TryCatch(async (req, res, next) => {
   ...
   if (!capsule)
      return next(
         new ErrorHandler(404, "Capsule not found or has been deleted")
      );
